...
if(isset($_SESSION['usr_id'])!="") {
	header("Location: \Movie DB new\index.php");
}
...
if (isset($_POST['login'])) {
	...
	if ($row = mysqli_fetch_array($result)) {
		...
		mysqli_query($connection, "UPDATE users SET last_login=now() WHERE email = '" . $email. "' and password = '" . md5($password) . "' ");

		if($row['user_type'] == 'Admin'){

				header("Location: /Movie DB new/responsive-sidebar-navigation/index.php");
		}
		else if(mysqli_query($connection, "SELECT * FROM users WHERE email = '" . $email. "' and password = '" . md5($password) . "' and user_type = 'general' "))
			header("Location: \Movie DB new\index.php");
	} 
	else {
...
<header class="site-header">
			<div class="container">
				<a href="\Movie DB new\index.php" id="branding">
					<img src="/Movie DB/images/logo.png" alt="" class="logo">
					<div class="logo-copy">
